...
public class PrimitiveTrail
{
    ...
    public List<Vector2> SmoothBezierPointRetreivalFunction(IEnumerable<Vector2> originalPositions, Vector2 generalOffset, int totalTrailPoints, IEnumerable<float> originalRotations = null)
    {
        ...
        BezierCurve bezierCurve = new BezierCurve(controlPoints.Where((Vector2 _, int i) => DegreeOfBezierCurveCornerSmoothening <= 0 || i % DegreeOfBezierCurveCornerSmoothening == 0 || i == 0 || i == controlPoints.Count - 1 || pointCountCondition).ToArray());
